...
public class WarehouseManager {
	...
	public static void main(String[] args) throws IOException {
		...
		//ここに入力処理を記述する。
		int choiceData = Integer.parseInt(br.readLine());

		System.out.print("\n要素数を選んでください（1...1個、2...2個、3...3個）＞");

		//ここに入力処理を記述する。
		int choiceElementsNum = Integer.parseInt(br.readLine());

		boolean errFlag = false;

		//ここに入力値の範囲チェック処理を記述する。
		if (choiceData < 1 || choiceData > 3 || choiceElementsNum < 1 || choiceData > 3) {
			System.out.println("Z先輩：");
		...
		if (!errFlag) {
			...
			//値の代入処理を記述する。
			if (choiceData == 1) {
				if (choiceElementsNum == 1) {
					charArray = new char[1];
					charArray[0] = 'a';
				} else if (choiceElementsNum == 2) {
			...
			if (choiceData == 1) {
				System.out.println(charArray[choiceElementsNum - 1]);
			} else if (choiceData == 2) {
				System.out.println(strArray[choiceElementsNum - 1]);
			} else {
				System.out.println(intArray[choiceElementsNum - 1]);
			}
